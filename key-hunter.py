...
DEFAULT_REGEX_PATTERN = r"""(?xi)\b(?:(?:api(?:_?key|_?secret))|(?:secret(?:_?key)?)|(?:(?:client|consumer|jwt)_?secret)|(?:(?:access|refresh|auth)_?token)|(?:token_?secret)|(?:oauth_consumer(?:_?key|_?secret))|(?:oauth_token(?:_?secret)?)|(?:(?:smtp|mail)_?password)|(?:(?:password|passwd|pwd))|(?:(?:private|public)_?key)|(?:(?:encryption|decryption)_?key)|(?:keystore(?:_?password)?)|(?:keyPassword)|(?:storePassword)|(?:aws_(?:access_key_id|secret_access_key|session_token))|(?:firebase_?api_?key)|(?:google(?:_api_key|_maps_key))|(?:sentry_?dsn)|(?:token)|(?:pass)|(?:_url)|(?:[A-Z0-9_]*?(?:URL|KEY|SECRET|PASSWORD|PASS|TOKEN)))\b\s*=\s*["'](?!android).*?["']"""
